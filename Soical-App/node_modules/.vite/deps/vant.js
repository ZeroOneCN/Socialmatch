...
var isIOS = () => inBrowser2 ? /ios|iphone|ipad|ipod/.test(navigator.userAgent.toLowerCase()) : false;
...
function useVisibilityChange(target, onChange) {
  if (!inBrowser2 || !window.IntersectionObserver) {
    return;
  }
  ...
  setup(props2, {
    ...
    const onScroll = () => {
      ...
      for (let i = 0; i < months.value.length; i++) {
        ...
        const visible = height <= bottom2 && height + heights[i] >= top2;
        if (visible) {
          visibleRange[1] = i;
          if (!currentMonth) {
            currentMonth = month;
...
var stdin_default69 = defineComponent({
  ...
  setup(props2, {
    ...
    const updateState = () => {
      ...
      state.value = {
        x: offset2.x > -1 ? offset2.x : windowWidth.value - width - props2.gap,
        y: offset2.y > -1 ? offset2.y : windowHeight.value - height - props2.gap,
        width,
        height
      };
    };
...
var stdin_default96 = defineComponent({
  ...
  setup(props2, {
    ...
    const isRenderCanvas = inBrowser2 ? hasCanvasSupport() : true;
    let canvasWidth = 0;
    let canvasHeight = 0;
    let canvasRect;
    const touchStart = () => {
    ...
    const initialize = () => {
      ...
      if (isRenderCanvas && canvasRef.value) {
        const canvas = canvasRef.value;
        const dpr = inBrowser2 ? window.devicePixelRatio : 1;
