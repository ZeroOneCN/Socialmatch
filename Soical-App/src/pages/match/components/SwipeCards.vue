  ...
  <div class="swipe-cards-container">
    ...
    <div class="cards-stack" v-else>
      <transition-group name="card">
        ...
        >
          <div class="card-content" @touchstart="handleContentTouch">
            <!-- 用户相册轮播 -->
            ...
            <div class="card-details" @touchstart="handleContentTouch">
              <!-- 匹配理由提示 -->
          ...
          <!-- 底部按钮区域 -->
          <div class="profile-button-container" @touchstart="handleContentTouch">
            <van-button 
              size="normal" 
              type="primary" 
...
const visibleUsers = computed(() => {
  const result = props.users.slice(currentIndex.value, currentIndex.value + 3);
  return result;
});
...
const getMatchReasons = (user) => {
  ...
  if (user.location && currentUser.value && currentUser.value.location && 
      (user.location === currentUser.value.location || 
       ...
       currentUser.location.includes(user.location))) {
    reasons.push("同城");
  }
...
const startDrag = (event, index) => {
  ...
  if (target.closest('.profile-button-container') || 
      target.closest('.van-button') ||
      target.closest('.card-swipe')) {
    return;
  }
...
const onDrag = (event) => {
  ...
  requestAnimationFrame(() => {
    ...
    // 如果垂直滑动距离大于水平滑动，则不处理卡片滑动
    if (Math.abs(deltaY) > Math.abs(deltaX) && Math.abs(deltaY) > 10) {
      return;
    }
    
    // 阻止默认行为，避免页面滚动
    if (event.cancelable && Math.abs(deltaX) > 10) {
...
const nextCard = () => {
  ...
  // 使用 nextTick 确保 DOM 更新后再进行后续操作
  nextTick(() => {
    // 检查是否需要加载更多
    if (currentIndex.value + 3 >= props.users.length) {
...
const viewProfile = (user) => {
  ...
  console.log('查看用户资料:', user.userId, user.nickname || user.username);
...
const handleContentTouch = (event) => {
  ...
  while (scrollableParent && scrollableParent !== document.body) {
    if (scrollableParent.classList.contains('card-details')) {
      const rect = scrollableParent.getBoundingClientRect();
      const isAtTop = scrollableParent.scrollTop === 0;
      const isAtBottom = scrollableParent.scrollTop + rect.height >= scrollableParent.scrollHeight;
